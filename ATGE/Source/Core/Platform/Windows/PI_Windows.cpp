...
namespace ATGE
{
    ...
    bool PlatformInterface::initPlatform(const WindowConfig& config)
    {
		Logger::debug("Starting up windows app...\n");

		// Create and store our Windows-specific state.
		PS_WindowsState* state = MemoryManager::allocate<PS_WindowsState>();
	...
	void PlatformInterface::ConsolePrintError(const char* message, u8 color)
	{
		// FATAL,ERROR,WARN,INFO,DEBUG,TRACE
		static const u8 levels[6] = { 64, 4, 6, 2, 1, 8 };
	...
	LRESULT WindowProc(HWND hwnd, u32 uMsg, WPARAM wParam, LPARAM lParam)
	{
		LRESULT result = 0;
